...
export function apply(ctx) {
  ...
  function settleRun(run) {
    ...
    releaseRun(run)
    // 同一轮集群可能被多个调用方等着（重复调用时后来的那个会搭上已有的一轮），
    // 所以结束时唤醒全部等待者，而不是只唤醒最初那个。
    const waiters = run.waiters.splice(0)
    for (const notify of waiters) notify()
  }

  /** 等待一轮集群结束；已经结束的集群立即返回。 */
  function waitForSettle(run) {
    if (run.settled) return Promise.resolve()
  ...
  /**
   * 一轮集群的「项目指纹」：全局目标 + 切片清单。
   *
   * 用来识别「同一个项目又被调用了一次」。只看 goal 与 tasks，不看并发数/验收开关——
   * 那些是执行参数，不改变「要做的是哪件事」。参数变了仍想真的重跑，走 `rerun: true`。
   */
  function runSignature(args) {
    const source = Array.isArray(args.tasks) ? args.tasks : []
    const tasks = source.map(raw => {
      const src = raw && typeof raw === 'object' ? raw : {}
      const files = Array.isArray(src.files) ? src.files.map(oneLine).join('|') : ''
      return [oneLine(src.title), oneLine(src.brief), files, oneLine(src.provider), oneLine(src.model)].join('\u0001')
    })
    return `${oneLine(args.goal)}\u0002${tasks.join('\u0003')}`
  }
  ...
  function createRun(parentSessionId, args, options) {
    ...
    return {
      ...
      childIds: [],
      report: '',
      /** 项目指纹：重复调用同一个项目时据此搭车/复用，而不是重新派发。 */
      signature: typeof options.signature === 'string' ? options.signature : '',
      waiters: [],
      detachOuter: null,
    }
  }
  ...
  ctx.tools.register(defineTool({
    name: 'swarm_run',
    description: '启动「并行子代理集群」：把整体目标拆成多个互不重叠的切片任务，按配置的并发数同时派发给多个独立子代理执行（各自独立会话，可分别指定模型），可选在全部完成后派发一名独立验收代理。返回每个切片的交付摘要与验收报告。用户界面会实时显示每个子代理正在做什么。注意：本工具对同一个目标+同一批切片是幂等的——重复调用不会重新派发，正在跑就搭上那一轮，已经成功跑完就直接返回已有报告；确实要强制重跑同样的切片时传 rerun: true。',
    parameters: {
      ...
      rerun: { type: 'boolean', description: '默认 false。同一个目标+同一批切片已经跑过并成功结束时，本工具会直接复用已有报告而不重新派发；只有确实要强制重跑一遍时才传 true' },
    },
    output: {
      schema: { type: 'json' },
      render: (_args, value) => [{
    ...
    async execute(args, exec) {
      ...
      const signature = runSignature(args)
      const force = args.rerun === true

      // ── 幂等护栏 ─────────────────────────────────────────────────────────────
      // 这里是本插件最容易出事的地方：`swarm_run` 只要被重复调用一次（模型重试、
      // 同一批工具调用被并发派发、上游续轮又问了同一件事），旧写法就会把整轮切片
      // 从零重新派发一遍——项目看起来「完成后又被完整完成一次」，而且会反复。
      // 现在按「项目指纹」判定，重复调用一律不再重新派发：
      //   · 同一项目正在跑     → 搭上那一轮并返回它的结果（绝不重启）
      //   · 同一项目已成功结束 → 直接复用已有报告，不新建任何子代理
      //   · 同一项目上次失败/被取消 → 放行，重试是合理的
      //   · 显式传 rerun: true → 跳过护栏，强制重跑
      const previous = runsByParent.get(parent.id)
      if (!force && previous !== undefined && previous.signature === signature && signature !== '') {
        if (!previous.settled) {
          if (args.wait === false) {
            return {
              ok: true,
              reused: true,
              report: `同一个目标已经有一轮集群在跑（${previous.id}），本次复用了它，没有重新派发子代理。用 swarm_status 查询进度。`,
              board: boardOf(previous),
            }
          }
          await waitForSettle(previous)
          return { ok: true, reused: true, report: reportText(previous), board: boardOf(previous) }
        }
        if (previous.status === 'done') {
          return {
            ok: true,
            reused: true,
            report: `集群 ${previous.id} 已经完成过完全相同的这批切片，本次没有重新派发任何子代理。\n\n`
              + `${previous.report}\n\n`
              + '如需强制重跑同样的切片，请显式传 rerun: true。',
            board: boardOf(previous),
          }
        }
      }
      ...
      if (previous !== undefined && !previous.settled) cancelRun(previous, '')
      ...
      const waiter = waitForSettle(run)
